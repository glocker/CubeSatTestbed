...
def test_wait_with_real_time_clock_paces_against_wall_clock_time() -> None:
    setup = load_testbed_config("configs/default_satellite.toml")
    runtime = build_runtime(setup, install_default_obc_rules=False, clock=RealTimeClock())
    runner = ScenarioRunner(runtime)
...
def test_wait_with_real_time_clock_delivers_an_early_frame_promptly() -> None:
    ...
    base_runtime = build_runtime(setup, install_default_obc_rules=False, clock=RealTimeClock())
